...
public:
  ...
  mds_gid_t find_standby_for(int mds, std::string& name) {
  ...
  mds_gid_t find_unused_for(int mds, std::string& name) {
    for (std::map<mds_gid_t,mds_info_t>::const_iterator p = mds_info.begin();
	 p != mds_info.end();
	 ++p) {
  ...
  mds_gid_t find_replacement_for(int mds, std::string& name) {
  ...
  int get_rank_gid(mds_gid_t gid) {
    if (mds_info.count(gid))
